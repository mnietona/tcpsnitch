...
cleanup() {
    ...
    # Optionnel : Fermer l'application sur le téléphone (décommenter si besoin)
...
# On "trap" le signal SIGINT (Ctrl+C) pour appeler le cleanup
trap cleanup SIGINT
...
echo "⏱️ Faites Ctrl+C à tout moment pour arrêter proprement."
echo "=================================================="

# On s'assure que le téléphone est dans le bon état de départ
if [ "$SCENARIO_INPUT" == "3" ] || [ "$SCENARIO_INPUT" == "blackout" ]; then
    # Pour le blackout, on doit couper la 4G dès le début, sinon la perte du WiFi
    # déclenchera un handover vers la 4G au lieu d'une coupure totale.
...
echo "[0s] 🟢 Démarrez votre activité (Scroll/Appel/Jeu) pendant 3 minutes..."
