...
case $APP_INPUT in
    1|telegram)
        APP_NAME="Telegram"
        APP_BIN="/home/matias/Telegram/Telegram"
        APP_ARGS=""
        KILL_NAME="Telegram"
        ;;
    2|vlc)
        APP_NAME="VLC"
        APP_BIN="vlc"
        # On passe directement le flux de Radio France
    ...
    3|transmission)
        ...
        # Pas besoin d'arguments spéciaux, l'interface s'ouvrira proprement
        APP_ARGS=""
...
# --- FONCTION DE FERMETURE PROPRE (CLEANUP) ---
cleanup() {
    echo -e "\n\n${RED}🛑 Fin du test ou Interruption détectée.${NC}"
    
    echo -e "${BLUE}🧹 Arrêt gracieux de TCPSnitch et $APP_NAME...${NC}"
    # On envoie SIGINT (Ctrl+C) pour que tcpsnitch vide ses buffers JSON proprement
    sudo pkill -INT tcpsnitch 2>/dev/null
    pkill -f "$KILL_NAME" 2>/dev/null
    
    echo -e "${BLUE}🌐 Restauration des paramètres réseaux...${NC}"
    nmcli networking on
    ...
    # CORRECTION ICI : Il manquait la commande et le 'fi'
    if [ ! -z "$ETH_DEV" ]; then
...
echo -e "⏱️  ${YELLOW}Faites Ctrl+C à tout moment pour arrêter proprement.${NC}"
echo "=================================================="
...
# Lancement de TCPSnitch avec les arguments dynamiques
...
if [ "$SCENARIO_INPUT" == "1" ]; then
    ...
    # SCENARIO 2 : HANDOVER
    if [ -z "$ETH_DEV" ]; then
        echo -e "${RED}⚠️ ATTENTION : Aucune carte Ethernet détectée ! Le Handover risque de ne rien faire.${NC}"
    fi
    sleep 90
...
echo -e "\n${PURPLE}====================================================${NC}"
echo -e "${PURPLE}[180s] 🟡 LEVEZ LES MAINS DU CLAVIER (Mettez le torrent en PAUSE)${NC}"
echo -e "${PURPLE}====================================================${NC}"
sleep 60
