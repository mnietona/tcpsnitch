...
By default, traces are saved locally in a `tcpsnitch_output` directory created where you run the command. Each execution generates a unique subdirectory stamped with the application name and the current date/time.

As visible on the next code snippet, `tcpsnitch` gives you the URL at which your trace is available.

```bash
$ tcpsnitch curl google.com
[tcpsnitch] Trace saved in: /home/user/tcpsnitch_output/curl_20231027_153000
```
...
- **Process subdirectories (by PID)**: One subdirectory per process, each containing the corresponding `sockets.json` trace files.
...
```bash
./bin/tcpsnitch curl google.com
```

One may issue `tcpsnitch -h` to get more information about the supported options. The most important ones are the following:

- `-d` sets the base directory in which traces will be written (defaults to `./tcpsnitch_output`).
- `-v` activates verbose mode. By default, the application's standard output (`stdout`/`stderr`) is hidden to keep the terminal clean. Use `-v` to see it.
...
- `-f` sets the verbosity level of logs saved to file.
- `-l` sets the verbosity of logs to `stderr` (internal tool errors).
...
`-b <bytes>` and `-u <usec>` allow to extract the value of the `TCP_INFO` socket option for each socket at user-defined intervals. Note that the `TCP_INFO` values appears as any other event in the JSON trace of the socket.
...
When both options are set, `TCP_INFO` is recorded when either one of the two conditions is matched.

## Defaults

- On Linux, `TCP_INFO` is captured every 100ms (100000 µs) by default.
- On Android, this feature is disabled by default (0) to avoid stability issues.
