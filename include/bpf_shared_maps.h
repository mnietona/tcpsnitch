...
/**
 * @brief Lookup key for the BPF hash map mapping open FDs to tcpsnitch sessions.
 */
struct fd_key {
    __u32 pid;
    __s32 fd;
...
/**
 * @brief Standardized event payload sent from the kernel to the user-space collector.
 */
struct ebpf_event {
    __u64         session_id;
    __u64         timestamp_ns;
...
// Size of the eBPF ring buffer (4 MB) to prevent drops under heavy network load.
