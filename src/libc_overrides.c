...
#define override(FUNCTION, RETURN_TYPE, ARGS_COUNT, ...)                       \
    ...
    EXPORT RETURN_TYPE FUNCTION(int fd, __VA_ARGS__) {                         \
        if (!orig_##FUNCTION)                                                  \
            orig_##FUNCTION = (FUNCTION##_type)dlsym(RTLD_NEXT, #FUNCTION);    \
    ...
    if (is_inet_socket(fd)) {
        ...
        if (ebpf_collector_is_active() &&
            ...
            if (orig_getsockname) {
                ...
                if (orig_getsockname(fd, (struct sockaddr *)&local_addr,
                                     &addrlen) == 0) {
...
EXPORT int ioctl(int fd, unsigned long int request, ...) {
    ...
    int err = errno;
    if (is_inet_socket(fd))
        sock_ev_ioctl(fd, ret, err, request);
...
EXPORT int poll(struct pollfd *fds, nfds_t nfds, int timeout) {
    ...
    for (i = 0; i < nfds; i++) {
        ...
        if (is_inet_socket(pollfd.fd))
            sock_ev_poll(pollfd.fd, ret, err, pollfd.events, pollfd.revents,
                         timeout);
    }
...
EXPORT int ppoll(struct pollfd *fds, nfds_t nfds, const struct timespec *tmo_p,
    ...
    for (i = 0; i < nfds; i++) {
        struct pollfd pollfd = fds[i];
        if (is_inet_socket(pollfd.fd))
            sock_ev_ppoll(pollfd.fd, ret, err, pollfd.events, pollfd.revents,
                          tmo_p);
    }
    ...
    for (fd = 0; fd < nfds; fd++) {
        if (is_inet_socket(fd) && req_ev[fd]) {
            sock_ev_select(fd, ret, err, (req_ev[fd] & READ_FLAG),
                           ...
                           exceptfds && FD_ISSET(fd, exceptfds), timeout);
        }
    }
...
EXPORT int epoll_wait(int epfd, struct epoll_event *events, int maxevents,
                      int timeout) {
    if (!orig_epoll_wait)
        orig_epoll_wait = (epoll_wait_type)dlsym(RTLD_NEXT, "epoll_wait");
    ...
    for (int i = 0; i < ret; i++) {
        ...
        if (is_inet_socket(fd))
            sock_ev_epoll_wait(fd, ret, err, timeout, events[i].events);
    }
...
EXPORT int epoll_pwait(int epfd, struct epoll_event *events, int maxevents,
    ...
    if (!orig_epoll_pwait)
        orig_epoll_pwait = (epoll_pwait_type)dlsym(RTLD_NEXT, "epoll_pwait");
    ...
    for (int i = 0; i < ret; i++) {
        int fd = events[i].data.fd;
        if (is_inet_socket(fd))
            sock_ev_epoll_pwait(fd, ret, err, timeout, events[i].events);
    }
