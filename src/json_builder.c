...
static json_t *build_sock_info(const SockInfo *sock_info) {
    ...
    if (sock_info->protocol)
        p = getprotobynumber(sock_info->protocol);
    if (p)
...
static json_t *build_msghdr(const Msghdr *msg) {
    ...
    if (msg->flags)
        add(json_msghdr, "flags", build_recv_flags(msg->flags));
...
static json_t *build_ip_mreqn(const struct ip_mreqn *ip_mreqn,
    ...
    if (includes_ifindex) {
        add(json_ip_mreqn, "imr_ifindex", json_integer(ip_mreqn->imr_ifindex));
        if (ip_mreqn->imr_ifindex != 0) {
            char *if_name = alloc_iface_name(fd, ip_mreqn->imr_ifindex);
...
static json_t *build_ipv6_mreq(const struct ipv6_mreq *ipv6_mreq, int fd) {
    ...
    if (ipv6_mreq->ipv6mr_interface != 0) {
        char *if_name = alloc_iface_name(fd, ipv6_mreq->ipv6mr_interface);
        add(json_ipv6_mreq, "ipv6mr_interface_name", json_string(if_name));
        free(if_name);
    }
...
static json_t *build_sol_ip_optval(const Sockopt *sockopt) {
    ...
    case IP_DROP_MEMBERSHIP:
        return build_ip_mreqn((struct ip_mreqn *)sockopt->optval,
                              sockopt->optlen == sizeof(struct ip_mreqn),
                              sockopt->fd);
    case IP_MULTICAST_IF:
        if (sockopt->getsockopt || sockopt->optlen == sizeof(struct in_addr)) {
            return build_in_addr(AF_INET, (struct in_addr *)sockopt->optval);
        } else {
            return build_ip_mreqn((struct ip_mreqn *)sockopt->optval,
                                  sockopt->optlen == sizeof(struct ip_mreqn),
                                  sockopt->fd);
    ...
    case IP_MULTICAST_TTL:
        return json_integer(*((unsigned char *)sockopt->optval));
...
static json_t *build_sol_ipv6_optval(const Sockopt *sockopt) {
    ...
    case IPV6_DROP_MEMBERSHIP:
        return build_ipv6_mreq((struct ipv6_mreq *)sockopt->optval,
                               sockopt->fd);
    case IPV6_MULTICAST_HOPS:
        return json_integer(*((int *)sockopt->optval));
    case IPV6_MULTICAST_IF: {
        char *if_name = alloc_iface_name(sockopt->fd, *(int *)sockopt->optval);
...
static json_t *build_sock_ev_netlink(const SockEvNetlink *ev) {
    ...
    if (ev->netlink_msg_type == RTM_NEWADDR)
        type_s = "NEW_ADDR";
    else if (ev->netlink_msg_type == RTM_DELADDR)
        type_s = "DEL_ADDR";
    else if (ev->netlink_msg_type == RTM_NEWROUTE)
        type_s = "NEW_ROUTE";
    else if (ev->netlink_msg_type == RTM_DELROUTE)
        type_s = "DEL_ROUTE";
    ...
    if (ev->family == AF_INET)
        add(json_details, "family", json_string("IPv4"));
    else if (ev->family == AF_INET6)
        add(json_details, "family", json_string("IPv6"));

    if (ev->ip_address)
        add(json_details, "ip", json_string(ev->ip_address));
...
static json_t *build_sock_ev(const SockEvent *ev) {
    ...
    case SOCK_EV_FORKED_SOCKET:
        r = build_sock_ev_forked_socket((const SockEvForkedSocket *)ev);
        break;
    ...
    case SOCK_EV_GETSOCKOPT:
        r = build_sock_ev_getsockopt((const SockEvGetsockopt *)ev);
        break;
    case SOCK_EV_SETSOCKOPT:
        r = build_sock_ev_setsockopt((const SockEvSetsockopt *)ev);
        break;
    case SOCK_EV_SEND:
    ...
    case SOCK_EV_GETSOCKNAME:
        r = build_sock_ev_getsockname((const SockEvGetsockname *)ev);
        break;
    case SOCK_EV_GETPEERNAME:
        r = build_sock_ev_getpeername((const SockEvGetpeername *)ev);
        break;
    case SOCK_EV_SOCKATMARK:
        r = build_sock_ev_sockatmark((const SockEvSockatmark *)ev);
        break;
    ...
    case SOCK_EV_EPOLL_PWAIT:
        r = build_sock_ev_epoll_pwait((const SockEvEpollPwait *)ev);
        break;
    case SOCK_EV_FDOPEN:
