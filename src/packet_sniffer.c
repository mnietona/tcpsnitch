...
static pcap_t *get_capture_handle(void) {
    ...
    if (err_buf[0] != 0)
        LOG(WARN, "pcap_open_live() warn. %s.", err_buf);
    if (!handle)
        goto error;
...
static void *capture_thread(void *params) {
    ...
    while (*switch_flag) {
        if (pcap_dispatch(args->handle, -1, &pcap_dump, (u_char *)args->dump) ==
            -1) {
...
char *alloc_capture_filter(const struct sockaddr *addr1,
    ...
    if (addr1) {
        if (!(port1 = alloc_port_str(addr1)))
            goto error_out;
        if (!(ip1 = alloc_ip_str(addr1)))
            goto error1;
    }
    if (addr2) {
        if (!(port2 = alloc_port_str(addr2)))
            goto error2;
        if (!(ip2 = alloc_ip_str(addr2)))
            goto error3;
    }
...
bool *start_capture(const char *filter_str, const char *path) {
    ...
    pthread_t thread;
    if (my_pthread_create(&thread, NULL, capture_thread, args))
        goto error4;
    return switch_flag;
error4:
    free(args);
